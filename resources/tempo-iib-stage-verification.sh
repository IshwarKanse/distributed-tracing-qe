...
# Note: v4.16 is excluded as its release pipeline failed
IIB_ENTRIES=(
    "v4.12:1108102"
    "v4.14:1108101"
    "v4.15:1108104"
...
OPERATOR_NAMESPACE="openshift-tempo-operator"
MARKETPLACE_NAMESPACE="openshift-marketplace"
CATALOGSOURCE_NAME="tempo-registry"
SUBSCRIPTION_NAME="tempo-product"
OPERATOR_PACKAGE="tempo-product"
...
spec:
  sourceType: grpc
  image: ${iib_image}
  displayName: Tempo Stage Registry
  publisher: Red Hat
  updateStrategy:
    registryPoll:
      interval: 15m
...
EOF
    ...
    for ((i=1; i<=retries; i++)); do
        local available
        available=$(oc get deployment tempo-operator-controller -n "$OPERATOR_NAMESPACE" -o jsonpath='{.status.availableReplicas}' 2>/dev/null || true)
        if [[ "$available" == "1" ]]; then
...
function run_verification_script() {
    log_info "Running tempo-images-and-version-check.sh..."
    bash "${SCRIPT_DIR}/tempo-images-and-version-check.sh"
}
...
# Main
echo "=============================================="
echo " Tempo IIB Stage Verification"
echo "=============================================="
